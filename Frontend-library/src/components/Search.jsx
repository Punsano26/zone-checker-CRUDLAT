...
const Search = ({ library, setFilteredLibraries }) => {  
  ...
  const handleChange = (e) => {
    ...
    const result = library.filter((library) => {
      return (
        library.title.toLowerCase().includes(e.target.value.toLowerCase()) ||
        library.author.toLowerCase().includes(e.target.value.toLowerCase())
      );
    });
